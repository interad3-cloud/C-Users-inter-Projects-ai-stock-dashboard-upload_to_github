...
def _apply_chart_layout(
    ...
    *,
    ...
    layout = dict(
        ...
        margin=dict(l=40, r=16, t=52 if title else 28, b=36),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
    ...
    if title:
        layout["title"] = dict(text=title, font=dict(size=14, color="#191F28"), x=0)
    if yaxis_title:
    ...
    fig.update_xaxes(showgrid=True, gridcolor=_CHART_COLORS["grid"], zeroline=False)
    fig.update_yaxes(showgrid=True, gridcolor=_CHART_COLORS["grid"], zeroline=False)
    return fig
...
def build_forecast_bar_chart(
    ...
    for metric, hist_color, fc_color in [
        ("매출액", "#3182F6", "rgba(49, 130, 246, 0.45)"),
        ("영업이익", "#26A69A", "rgba(38, 166, 154, 0.45)"),
    ]:
...
def build_revenue_profit_bar(chart_df: pd.DataFrame) -> go.Figure | None:
    ...
    fig = go.Figure()
    for metric, color in [("매출액", "#3182F6"), ("영업이익", "#26A69A")]:
